...
public class CreateServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (u != null && u.getRole() == 1) {
                ...
                String[] strInput = {image, productId, productName, description, 
                quantity, Year, unit,price};
    ...
    private boolean IsValidate(String[] strIn, String[] strOut) throws Exception{
        ...
        iCheck++;
        ProductDAO checkProductDAO = new ProductDAO();
        
        Product checkProduct = checkProductDAO.getProductById(strIn[iCheck]);
        if(checkProduct != null){        
            if (checkProduct.getProductId().equals(strIn[iCheck])) {
                strOut[iCheck] = "Product ID is already existed";
            } else {
                strOut[iCheck] = null;
            }
        }else if(!strIn[iCheck].isEmpty()){
            String fisrt = String.valueOf(strIn[iCheck].trim().charAt(0));
            if (!strIn[iCheck].matches("\\d{3,9}")) {
                strOut[iCheck] = "Product ID must be  contain at least 3 digits";
            } else {
                strOut[iCheck] = null;
            }
        }
        ...
        iCheck++;
        if(checkProduct != null){
            if (checkProduct.getProductName().equals(strIn[iCheck])) {
                strOut[iCheck] = "Product Name is already existed";
            } else {
                strOut[iCheck] = null;
            }       
        }else if(!strIn[iCheck].isEmpty()){
